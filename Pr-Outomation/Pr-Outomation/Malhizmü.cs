...
{
    public partial class Malhizmü : Form
    {
        public Malhizmü()
...

        {
            ...
            da = new SqlDataAdapter("Select *From Malhizmü", con);
            ds = new DataSet();
            con.Open();
            da.Fill(ds, "Malhizmü");
            dataGridView1.DataSource = ds.Tables["Malhizmü"];
            con.Close();
        }
...

        {
                ...
                con.Open();
                cmd.Connection = con;
                cmd.CommandText = "INSERT INTO Malhizmü (Yazıcı,Model,Toner,Tarih) values (@Yazıcı,@Model,@Toner,@Tarih)";
...

        {
            ...
            con.Open();
            cmd.Connection = con;
            cmd.CommandText = "UPDATE Malhizmü SET Yazıcı=@Yazıcı,Model=@Model,Toner=@Toner,Tarih=@Tarih where Yazıcı=@Yazıcı";
            cmd.Parameters.AddWithValue("@Yazıcı", YazıcıTBox.Text);
            cmd.Parameters.AddWithValue("@Model", Toner_ModelTBox.Text);
            cmd.Parameters.AddWithValue("@Toner", comboBox1.Text);
            cmd.Parameters.AddWithValue("@Tarih", dateTimePicker1.Text);
...

        {
            ...
            cmd.CommandText = "DELETE FROM [Malhizmü] WHERE Yazıcı=@Yazıcı";
        ...
        private void Malhizmü_Load(object sender, EventArgs e)
